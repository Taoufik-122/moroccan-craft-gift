...
const ProductDetail = () => {
  ...
  // تحديث الصورة الرئيسية بناءً على الفارييشن أو الصور الأساسية
  useEffect(() => {
    if (!selectedVariations || Object.keys(selectedVariations).length === 0) {
      setMainImage(productImages[0]?.image_url || product?.image_url || null);
      return;
    }

    const variationImage = Object.values(selectedVariations)
      .find((v: any) => v?.image_url)?.image_url;
  ...
  }, [selectedVariations, productImages, product]);
  ...
  const getProductDescription = (product: Product) => {
    switch (language) {
      case 'ar': return product.description_ar;
      case 'fr': return product.description_fr;
  ...
  const handleAddToCart = (product: Product) => {
    ...
    const variationAdjustment = Object.values(selectedVariations).reduce(
      (total: number, variation: any) => total + variation.price_adjustment, 0
    );
  ...
  if (loading) return (
    <div className="min-h-screen bg-gradient-subtle flex items-center justify-center">
      <div className="text-center animate-pulse text-xl">Loading product...</div>
    </div>
  );
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 mb-16">
          ...
          <div className="relative">
            <img 
              src={mainImage || product.image_url || 'https://images.unsplash.com/photo-1618160702438-9b02ab6515c9?w=500'}
            ...
            <div className="flex gap-2 mt-4 overflow-x-auto">
              {productImages.length > 0 ? productImages.map((img) => (
                <img
                  ...
                  src={img.image_url}
                  alt={getProductName(product)}
                  className="w-24 h-24 object-cover rounded-lg cursor-pointer border hover:border-primary transition-all"
          ...
          <div className="space-y-6">
            ...
            <div className="space-y-4">
              ...
              <p className="text-muted-foreground leading-relaxed whitespace-pre-wrap">
                {getProductDescription(product) || "This beautiful handcrafted item represents the finest artisanal traditions of Morocco."}
              </p>
            </div>
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4 text-sm">
                <div><span className="font-medium">Category:</span> <p className="text-muted-foreground">{getCategoryName(product.categories)}</p></div>
                <div><span className="font-medium">Origin:</span> <p className="text-muted-foreground">{product.categories.city}</p></div>
                <div><span className="font-medium">Stock:</span> <p className="text-muted-foreground">{product.stock_quantity} available</p></div>
                <div><span className="font-medium">SKU:</span> <p className="text-muted-foreground">{product.id.slice(0, 8).toUpperCase()}</p></div>
              </div>
            </div>
              ...
              {similarProducts.map((similarProduct) => (
                ...
                >
                  ...
                  <CardContent className="p-4">
                    <h3 className="font-semibold mb-2 group-hover:text-primary transition-colors">{getProductName(similarProduct)}</h3>
